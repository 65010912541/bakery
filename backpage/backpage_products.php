...
function redirect_to(string $tab, int $page = 1): void {
  // แยกไฟล์แล้ว: สินค้าเด้งกลับหน้า products file เสมอ
  header("Location: backpage_products.php?page=" . $page);
  exit;
}
...
/* ===== ใช้สำหรับ select ใน modal: ดึงหมวดทั้งหมด (กันหมวดหาย) ===== */
$categoriesForSelect = $pdo->query("SELECT id, name, slug FROM categories ORDER BY name ASC")->fetchAll();
  ...
  <div class="card p-3 p-md-4">
    ...
    <div class="table-responsive mt-3">
      <div class="table-headbar d-flex align-items-center justify-content-between mb-2">
        <div class="text-muted small">
          ทั้งหมด <span class="chip bg-light border"><?= (int)$totalProducts ?></span> รายการ
        </div>
...
<form id="hiddenDeleteForm" method="post" class="d-none">
  ...
  <input type="hidden" name="back_tab" id="hd_back_tab" value="products">
        ...
        <div class="row g-3">
          <div class="col-md-6">
            ...
            <select class="form-select" name="category_id" required>
              ...
              <?php foreach ($categoriesForSelect as $c): ?>
